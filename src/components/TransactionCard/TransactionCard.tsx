      ...
      <div className="card w-50 mx-auto p-4 mb-3">
        <div className="card-body">
          ...
          <p className="card-text">
            {transaction.type === 'income' ? (
              <span className="text-success fs-3">
                <b>+{transaction.transactionSum} KGS</b>
              </span>
            ) : (
          ...
          <p className="card-text fs-3">
            <b>Category:</b> {categoryFromTransaction}
          </p>
          <p className="card-text fs-3">
            <b>Type:</b> {transaction.type}
          </p>
          <p className="card-text fs-3">
            <b>Date:</b> {dayjs(transaction.date).format('DD.MM.YYYY HH:mm:ss')}
          </p>
        </div>

        <Link
          to={`/transactions/edit-transaction/${transaction.id}`}
